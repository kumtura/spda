...
<div class="bg-white pb-24">
    ...
    <div class="px-4 pt-4 space-y-4">
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-5">
            <form action="{{ url('administrator/kelian/tiket/jual/submit') }}" method="POST" id="jual-form">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <!-- Submit Button -->
                    <button type="submit" id="submit-btn" class="w-full py-3 bg-[#00a6eb] text-white text-sm font-black rounded-xl shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed">
...
function loadKategori() {
    ...
    if (!objek || objek.kategori.length === 0) {
        container.classList.add('hidden');
        alert('Objek wisata ini belum memiliki kategori tiket. Silakan tambahkan kategori terlebih dahulu.');
...
document.getElementById('jual-form').addEventListener('submit', function(e) {
    ...
    // Add hidden inputs for kategori
    Object.values(selectedKategori).forEach(kat => {
        if (kat.qty > 0) {
