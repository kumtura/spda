...
<div class="bg-white px-4 pt-8 pb-24 space-y-6" x-data="{ 
    ...
    <!-- Back + Header -->
    <div>
        <a href="{{ url('administrator/kelian/pendatang/detail/'.$pendatang->id_pendatang) }}" class="inline-flex items-center gap-1.5 text-slate-400 hover:text-[#00a6eb] transition-colors mb-3">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <!-- Print Button -->
                <a :href="'{{ url('administrator/kelian/pendatang/kartu-punia/print/'.$pendatang->id_pendatang) }}?year=' + selectedYear" 
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors {{ !$isPaid && $isPast ? 'bg-rose-50/30' : '' }}">
                    ...
                    <td class="px-2 py-3.5 text-center">
                        @if($isPaid)
                        <span class="text-[10px] text-slate-500">{{ $payment->tanggal_bayar->format('d/m') }}</span>
                        @else
                        <span class="text-[10px] text-slate-300">-</span>
                        @endif
                    </td>
