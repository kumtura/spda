...
@php
    ...
    $puraList = \App\Models\Pura::with('banjar')->where('aktif', '1')->orderBy('nama_pura')->get();
...
<div class="bg-white rounded-t-3xl -mt-4 relative z-20 px-4 pt-8 pb-10 space-y-8">
    ...
    <div>
        <div class="flex items-center justify-between mb-4 px-1">
            <h3 class="text-sm font-bold text-slate-800">Punia Pura</h3>
            @if(count($puraList) > 0)
                <a href="{{ route('public.pura') }}" class="text-[10px] font-bold text-[#00a6eb] hover:underline">Lihat Semua</a>
            @endif
        </div>
            ...
            <div class="flex gap-3 overflow-x-auto no-scrollbar pb-2 -mx-4 px-4">
                @foreach($puraList as $pura)
                    <a href="{{ route('public.pura.detail', $pura->id_pura) }}" class="flex-shrink-0 w-56 bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-all group">
                        ...
                        <div class="p-3">
                            <h4 class="text-xs font-bold text-slate-800 leading-tight mb-1 group-hover:text-[#00a6eb] transition-colors line-clamp-2">{{ $pura->nama_pura }}</h4>
                            @if($pura->banjar)
                            <p class="text-[10px] text-slate-400 mb-2">{{ $pura->banjar->nama_banjar }}</p>
                            @endif
                            <span class="bg-slate-50 text-slate-600 group-hover:bg-[#00a6eb] group-hover:text-white transition-colors px-3 py-1.5 rounded-full text-[10px] font-bold inline-block">Punia</span>
                        </div>
                    </a>
                @endforeach
            </div>
        @else
            <div class="bg-slate-50 rounded-2xl border border-slate-100 border-dashed p-6 text-center">
                <i class="bi bi-building text-3xl text-slate-300 mb-2 block"></i>
                <p class="text-xs font-medium text-slate-500">Belum ada data pura.</p>
            </div>
        @endif
    </div>
</div>
