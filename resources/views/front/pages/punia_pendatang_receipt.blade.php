...
@php
    ...
    $downloadUrl = route('public.punia.pendatang.receipt.download', ['code' => $receiptCode]);
@endphp
...
<div class="bg-white min-h-screen pb-8" x-data="{ shareOpen: false, waNumber: '' }">
    <div class="bg-gradient-to-br from-[#00a6eb] to-[#0090d0] px-4 pt-8 pb-12 text-white relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <p class="text-white/80 text-xs">Bukti pembayaran punia pendatang</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="p-6 border-b border-slate-100">
                <div class="flex items-center justify-between gap-3 mb-4">
                    <div>
                        ...
                        <h2 class="text-lg font-black text-slate-800">{{ $pendatang->nama }}</h2>
                    </div>
                ...
                <div class="bg-slate-50 rounded-2xl p-4 space-y-3">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span class="text-xs font-bold text-slate-800 text-right">{{ $pendatang->banjar->nama_banjar_adat ?? '-' }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                        <span class="text-xs text-slate-500">Periode</span>
                        <span class="text-xs font-bold text-slate-800 text-right">{{ $bulanName }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                        <span class="text-xs text-slate-500">Tanggal Bayar</span>
                        <span class="text-xs font-bold text-slate-800 text-right">{{ $payment->tanggal_bayar ? \Carbon\Carbon::parse($payment->tanggal_bayar)->translatedFormat('d M Y') : '-' }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                        <span class="text-xs text-slate-500">Metode</span>
                        <span class="text-xs font-bold text-slate-800 text-right">{{ ucfirst($payment->metode_pembayaran ?? '-') }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                        <span class="text-xs text-slate-500">Nominal</span>
                        <span class="text-sm font-black text-emerald-600 text-right">Rp {{ number_format($payment->nominal, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 space-y-3">
                ...
                <div x-show="shareOpen" x-cloak class="bg-slate-50 rounded-2xl border border-slate-200 p-4 space-y-3">
                    ...
                    <button type="button" onclick="sendPuniaPendatangReceiptToWhatsApp()" class="w-full py-3 bg-emerald-500 text-white rounded-xl font-bold text-sm">
                        Buka WhatsApp
                    </button>
                </div>
            </div>
...
<script>
    ...
    function sendPuniaPendatangReceiptToWhatsApp() {
