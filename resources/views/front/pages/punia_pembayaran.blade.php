...
<div class="bg-white pb-24">
    ...
    <div class="px-4 -mt-6 relative z-10">
        <form action="{{ route('public.punia.pembayaran.submit') }}" method="POST" class="bg-white rounded-3xl shadow-xl border border-slate-100 p-6 space-y-6" x-data="{ isAnonymous: false }">
            @csrf
            
            <!-- Anonymous Toggle -->
            <div class="bg-blue-50 border border-blue-100 rounded-xl p-4">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" x-model="isAnonymous" class="h-5 w-5 text-[#00a6eb] rounded">
                    <div class="flex-1">
            ...
            <div class="space-y-4" x-show="!isAnonymous" x-transition>
                ...
                <div class="space-y-1.5">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Nomor WhatsApp <span class="text-rose-500">*</span></label>
                    <input type="tel" name="no_wa" :required="!isAnonymous"
                           class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-slate-700 outline-none focus:ring-4 focus:ring-[#00a6eb]/10 transition-all"
            ...
            <div class="bg-blue-50 border border-blue-100 rounded-xl p-4">
                <div class="flex items-start gap-3">
                    ...
                    <p class="text-[10px] text-slate-600 leading-relaxed">Setelah submit, Anda akan diarahkan ke halaman payment gateway untuk menyelesaikan pembayaran.</p>
                </div>
            </div>
