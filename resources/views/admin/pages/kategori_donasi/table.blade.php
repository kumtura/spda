...
<div class="space-y-6" x-data="{ 
    ...
    @open-modal.window="show = true; isEdit = false; form = { id_kategori_donasi: '', nama_kategori: '', deskripsi_singkat: '' }"
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-black text-slate-800 tracking-tight">Kategori Program Donasi</h1>
            <p class="text-slate-500 font-medium text-sm">Manajemen pengelompokan program bantuan dan donasi sosial.</p>
        </div>
                        ...
                        <tr class="group hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button type="button" 
                                            @click="$dispatch('edit-modal', { id_kategori_donasi: '{{ $item->id_kategori_donasi }}', nama_kategori: '{{ e(str_replace("'", "\\'", $item->nama_kategori)) }}', deskripsi_singkat: '{{ e(str_replace(["\r\n", "\r", "\n", "'"], ["\\n", "\\n", "\\n", "\\'"], $item->deskripsi_singkat ?? "")) }}' })"
                                    ...
                                    <a href="{{ url('administrator/kategori_donasi/hapus/'.$item->id_kategori_donasi) }}" 
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="p-8 text-center">
                                ...
                                <span class="text-sm font-semibold text-slate-500">Belum ada Kategori Donasi.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <template x-teleport="body">
        <div x-show="show" 
            ...
            <div class="bg-white w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl relative border border-slate-200" @click.away="show = false">
                ...
                <form :action="isEdit ? '{{ url('administrator/kategori_donasi/update') }}' : '{{ url('administrator/kategori_donasi/post') }}'" method="POST" class="p-6 space-y-6">
                    @csrf
                    <input type="hidden" name="id_kategori_donasi" x-model="form.id_kategori_donasi">
