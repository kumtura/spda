    ...
    <form method="POST" action="{{ url('wisata/payment/proceed') }}" class="px-4 -mt-6 relative z-10 space-y-4">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-xl border border-slate-100 space-y-4">
            ...
            <div class="pt-3 border-t border-slate-100 space-y-2">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ringkasan Tiket</p>
                @foreach($tiketData['kategori'] as $item)
                <div class="flex items-center justify-between text-xs">
                    <span class="text-slate-600">{{ $item['nama_kategori'] }} x{{ $item['jumlah'] }}</span>
        ...
        <div class="space-y-4">
            ...
            <!-- Instant Payment -->
            @if($instantPayments->count() > 0 && $is_configured)
            <div class="space-y-3">
                <div class="flex items-center justify-between px-1">
            ...
            <!-- Virtual Account -->
            @if($vaPayments->count() > 0 && $is_configured)
            <div class="space-y-3">
                <div class="flex items-center justify-between px-1">
