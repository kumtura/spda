...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('tb_punia_pura', function (Blueprint $table) {
            ...
            $table->string('metode_pembayaran', 50)->nullable(); // xendit, qris_bpd, manual
            $table->string('xendit_id', 255)->nullable();
            $table->text('payment_data')->nullable();
            $table->string('status_pembayaran', 30)->default('pending'); // pending, completed, failed
            $table->date('tanggal_pembayaran')->nullable();
            $table->string('keterangan', 255)->nullable();
            $table->string('bukti_transfer', 255)->nullable();
            $table->string('status_verifikasi', 30)->default('pending'); // pending, verified, rejected
        ...
        Schema::create('tb_qris_pura', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('id_pura');
            $table->string('qris_content', 500)->nullable(); // raw QRIS string from BPD Bali
            $table->string('qris_image', 255)->nullable(); // stored QR image file path
            $table->string('nmid', 100)->nullable(); // National Merchant ID
